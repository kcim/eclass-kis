...
<input type="hidden" name="question_types[]" class="input" value="mcmultiple"/>
<input type="hidden" name="question_choice_count[]" class="input" value="<?=sizeof($question['options'])?>"/>

<? foreach ($question['options'] as $i=>$option): ?>
    <div>
    <input type="checkbox" <?=$attr?> <?=in_array($i, $answers)? 'checked="checked"': ''?> value="<?=$i?>" name="answers[<?=$question['seq']?>][]">
...
<? endforeach; ?>
<a class="add_choice_multiple input" href="#"><?=$kis_lang['addchoice']?></a>
